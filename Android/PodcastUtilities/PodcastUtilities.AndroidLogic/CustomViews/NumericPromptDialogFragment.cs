...
{
    public class NumericPromptDialogFragment : AndroidX.Fragment.App.DialogFragment
    {
        ...
        public struct NumericPromptDialogFragmentParameters
        {
            public string Title;
            public string Ok;
        ...
        public static NumericPromptDialogFragment NewInstance(NumericPromptDialogFragmentParameters parameters)
        {
            var dialog = new NumericPromptDialogFragment();
            var args = new Bundle();
            ...
            args.PutLong(VALUE_KEY, parameters.Value);
            dialog.Arguments = args;
            return dialog;
        }

        // tag, customData, value
        public EventHandler<Tuple<string, string, long>> OkSelected;
        public EventHandler<Tuple<string, string, long>> CancelSelected;
        ...
        {
            var view = inflater.Inflate(Resource.Layout.dialog_fragment_numeric_prompt, container);
            txtTitle = view.FindViewById<TextView>(Resource.Id.numeric_prompt_title);
            layValue = view.FindViewById<TextInputLayout>(Resource.Id.numeric_prompt_value_layout);
            txtValue = view.FindViewById<EditText>(Resource.Id.numeric_prompt_value);
            okButton = view.FindViewById<Button>(Resource.Id.numeric_prompt_ok);
            cancelButton = view.FindViewById<Button>(Resource.Id.numeric_prompt_cancel);
            clearButton = view.FindViewById<ImageView>(Resource.Id.numeric_prompt_txt_clear);
            return view;
        }
        ...
        public override void OnViewCreated(View view, Bundle savedInstanceState)
        {
            ...
            var value = args.GetLong(VALUE_KEY);
        ...
        {
            CancelSelected?.Invoke(this, Tuple.Create(tag, customData, ConvertResult()));
            ExitDialog();
        }

        private void DoOkAction(string tag, string customData)
        {
            OkSelected?.Invoke(this, Tuple.Create(tag, customData, ConvertResult()));
            ExitDialog();
        }

        private long ConvertResult()
        {
            return long.Parse(txtValue.Text);
        }
